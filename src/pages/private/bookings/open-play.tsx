...
const initialSessions: OpenPlaySession[] = [
	{
		...
		participants: [
			{ id: "u1", name: "Alice Johnson", status: "Resting" },
			{ id: "u2", name: "Bob Smith", status: "Resting" },
			{ id: "u3", name: "Carol Davis", status: "Resting" },
			{ id: "u4", name: "David Lee", status: "Resting" },
		],
	},
	{
		id: "op-2",
...
const OpenPlayPage: React.FC = () => {
  ...
  const [scoreEntry, setScoreEntry] = useState<Record<string, string>>({});
	...
	const players: Player[] = useMemo(() => {
		...
		return selectedSession.participants.map((p) => ({ id: p.id, name: p.name, team: "Open Play", status: p.status, avatarUrl: undefined }));
	}, [selectedSession]);

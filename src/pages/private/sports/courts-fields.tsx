...
const CourtsFieldsPage: React.FC = () => {
  ...
  const filtered = useMemo(() => {
    ...
    if (query.trim()) {
      ...
      list = list.filter((i) =>
        [i.name, i.type, i.location, i.status].some((v) =>
  ...
  function openEdit(it: Facility) {
    ...
    setForm({
      ...
      status: it.status,
      images: it.images && it.images.length ? it.images : [""],
      capacity: it.capacity ?? 0,
      openingHours: it.openingHours ?? "",
      reservations: it.reservations ?? 0,
      amenities: it.amenities && it.amenities.length ? it.amenities : [""],
    });
    setOpen(true);
  }
  function save(e: React.FormEvent) {
    ...
    if (editing)
      setItems((prev) =>
        prev.map((i) => (i.id === editing.id ? { ...editing, ...payload } : i))
      );
  ...
  const onDrop = useCallback((acceptedFiles: File[]) => {
    ...
    setForm((p) => ({
      ...p,
      images: [...(p.images ?? []).filter(Boolean), ...urls],
    }));
  }, []);
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <div className="mt-2 flex items-center gap-2 text-xs text-muted-foreground">
                    {typeof f.capacity === "number" && (
                      <span>Capacity: {f.capacity}</span>
                    )}
                    {f.openingHours && <span>• Hours: {f.openingHours}</span>}
                    {typeof f.reservations === "number" && (
                      <span>• Reservations: {f.reservations}</span>
                    )}
                  </div>
                </div>
              </div>
          ...
          <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4">
            <label className="space-y-1">
              ...
              <Input
                ...
                step="0.01"
                onChange={(e) =>
                  setForm((p) => ({ ...p, hourlyRate: Number(e.target.value) }))
                }
              />
            </label>
            ...
            <div className="space-y-2">
              <div
                ...
                className={`rounded-md border border-dashed p-4 text-center text-sm ${
                  isDragActive ? "bg-muted/50" : "bg-transparent"
                }`}
              >
                <input {...getInputProps()} />

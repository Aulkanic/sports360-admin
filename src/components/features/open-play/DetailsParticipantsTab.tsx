...
import {
  getStatusString,
  getSkillLevelAsLevel,
} from "@/components/features/open-play/types";
import AddPlayerModal, {
  type PlayerFormData,
} from "@/components/features/open-play/AddPlayerModal";
import {
  Users,
  Trophy,
  UserCheck,
                ...
                <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
                  ...
                  <div className="text-center">
                    <div className="text-2xl font-bold text-blue-600">
                      {
                        participants.filter(
                          (p) => getStatusString(p.status) === "IN-GAME"
                        ).length
                      }
                    </div>
              ...
              <div className="p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="text-sm text-gray-500 bg-blue-50 px-3 py-1 rounded-lg">
                    💡 Click buttons to change player status: Ready • Resting •
                    Waitlist • Reserve • Reject (IN-GAME and Endgame status
                    cannot be changed)
                  </div>
                </div>
                            ...
                            {!["READY"].includes(
                              ...
                              !["IN-GAME"].includes(
                                ...
                                <Button
                                  ...
                                  disabled={isUpdatingStatus.has(
                                    participant.id
                                  )}
                                  className="text-green-700 border-green-300 hover:bg-green-50"
                                >
                                  <UserCheck className="h-3 w-3 mr-1" />
                                  {isUpdatingStatus.has(participant.id)
                                    ? "Updating..."
                                    : "Ready"}
                                </Button>
                              )}

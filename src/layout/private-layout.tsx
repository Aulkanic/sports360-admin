...
const PrivateLayout = () => {
  ...
  const isFullScreenRoute = location.pathname === urls.openPlay;
  return (
    <SidebarProvider className="h-screen overflow-hidden">
      {!isFullScreenRoute && <AppSidebar className="!bg-white" />}
      <SidebarInset>
        {!isFullScreenRoute && (
        ...
        <div className={isFullScreenRoute ? "flex-1 bg-input overflow-y-auto overflow-x-hidden h-screen p-0" : "flex-1 p-4 bg-input overflow-y-auto overflow-x-hidden h-[calc(100vh-5rem)]"}>
          <Outlet />
        </div>
      </SidebarInset>
    </SidebarProvider>
